...
var appViewSes = function appViewSes(statusbox) {
    ...
    this.initialize = function() {
        ...
        this.detailsbox = new detailsBox(appview,"Sesiones");
...
//        this.getSessions();
    ...
    this.hover = function(s) {
        this.detailsbox.session(s,'Sesión');
    }

    this.refreshDetails = function() {
        if(this.selectedSessions.length > 1) this.detailsbox.sessions(this.selectedSessions.length);
        else if(this.selectedSessions.length == 1) this.detailsbox.session(this.selectedSessions[0],'Sesión');
        else this.detailsbox.category(this.selectedCategory.name,this.sessions.length,"Categoría");
    }
...
var appViewCat = function appViewCat(statusbox) {
    ...
    this.initialize = function() {
        ...
        this.listbox.elementsList();
        this.detailsbox = new detailsBox(this,"Categorías");
        this.detailsbox.animate();
        this.emptySessions = new session(null,"0","Ning\u00FAn resultado encontrado",null,appview);
        this.getSessions();
    ...
    this.hover = function(s) {
        this.detailsbox.session(s,'Categoría');
    }

    this.refreshDetails = function() {
        if(this.selectedSessions.length > 1) this.detailsbox.sessions(this.selectedSessions.length);
        else if(this.selectedSessions.length == 1) this.detailsbox.session(this.selectedSessions[0],'Categoría');
        else this.detailsbox.category(this.selectedCategory.name,this.sessions.length,"Categorías");
    }
...
var appViewTrainBox = function appViewTrainBox(statusbox) {
    ...
    this.train = function() {
        if(G_Tables['polarity'].fnGetData().length > 10) {
        appview.createClassiffier();
        $.ajax({
            url:'php_to_java.php',
            data: {
                text:"START_TRAIN:" + V_TRAINER.contexts[1].id + ":" + appview.traingroup
            },
            success: function(data) {
                if(data == "error: could not connect to host\n") {
                    V_STATUSBOX.showMessage('error',"¡¡Debes arrancar el servidor java de entrenamiento!!");                    
                }
                else if(data == "error: failed to write to socket\n") {
                    V_STATUSBOX.showMessage('error',"¡¡Ocurrió un problema de comunicación con el servidor de entrenamiento!!");                                        
                }
                else {
                    $('a',V_TRAINER.contexts[1].viewselector.element).each(function() {
                        if($(this).text() == "Clasificadores") $(this).click();
                    });
                    V_STATUSBOX.showMessage('info',"Ha comenzado el entrenamiento con el conjunto <b>" + appview.traingroup + "</b>");
                }
            }
        });        
    }
    else V_STATUSBOX.showMessage('alert',"Son necesarios <b>diez elementos</b> como mínimo para realizar la evaluación del clasificador");
 }
...
var appViewTrainClassifiers = function appViewTrainClassifiers(statusbox) {
    var appview = this;
    this.info = "\u00BFQuieres gestionar los clasificadores? Lo puedes hacer todo aqu\u00ED, desde accder a su información, hasta compararlos en una gráfica"
    ...
    this.initialize = function() {
        ...
        this.detailsbox = new detailsBox(this,"Clasificadores");
        this.detailsbox.animate();
        this.detailsbox.category("",this.sessions.length,"Clasificadores");
    ...
    this.hover = function(s,aData) {
        this.detailsbox.classifier(aData);
    }

    this.refreshDetails = function(table,aData) {
        var selectedElements = table.fnGetSelected('xtr_select');
        if(selectedElements.length > 1) this.detailsbox.sessions(selectedElements.length);
        else if(selectedElements.length == 1) this.detailsbox.classifier(table.fnGetData(selectedElements[0]));
        else this.detailsbox.category("",table.fnGetSelected('xtr').length);
    }
    ...
    this.deleteElements = function(table) {
        var selectedElements = table.fnGetDataSelected('xtr_select');
        //console.log(selectedElements);
        if(selectedElements.length == 0) V_STATUSBOX.showMessage('alert',"Debes seleccionar al menos un elemento");
        else {
            $(selectedElements).each(function() {
                console.log(this._aData[4]);
                var id = this._aData[4];
                $.ajax({
                    url:'classifiers.php',
                    data: {
                        mode:"delete",
...
var tools = function tools(appview) {
    ...
    this.addCollapse = function() {
        this.toolbox.addCollapse();        
    }
...
//        for(i in appview.categories) {
    ...
    this.addTrain = function() {
        this.actions.addTrain();
    }
    
    this.addCollapse = function() {
        this.selectors.addCollapse();        
    }
...
//        var translate = new translateBox(appview);
    ...
    this.addTrain = function() {
        var trainButton = $('<div id=train_button></div>');
        a.form.append(trainButton);
...
var selectors = function selectors(father,appview) {
    ...
    this.addCollapse = function() {
        this.element.append('   Grupos: <a id="colapseAll" href=" ">Contraer todos</a>, <a id="colapseNone" href=" ">Expandir todos</a>');        
        $("#colapseAll",this.element).click(function() {
            //colapse all the groups
            //appview.colapseAll();
            $(".open",$(".listwrap:visible")).click();
            return false;
        });

        $("#colapseNone",this.element).click(function() {
            //expand all the groups
            //appview.colapseNone();
            $(".close",$(".listwrap:visible")).click();
            return false;
        });
    }
}
...
//            'bProcessing':true,
            ...
            },
            {
                ...
                "aTargets":[3],
                "fnRender" : function(oObj) {
                    if(oObj['aData'][16] < 100) {
                        var cell = '<p class="progressBar"><span><em style="left:' + 1.28*parseInt(oObj['aData'][16]) + 'px">' + oObj['aData'][16] + '%</em></span></p>';
                        var refreshPB = function refreshPB() {
                            $.ajax({
                                url:'classifiers.php',
                                dataType:'json',
                                data: {
                                ...
                                success: function(json) {
                                    G_Tables[group].fnUpdate(json['aaData'],oObj.iDataRow,false,false);
                                }
                            });
                        }
                        setTimeout(refreshPB,2000);            
                        redraw = true;
                        return cell;
                    }
                    else {
            ...
            "fnRowCallback": function( nRow, aData, iDisplayIndex ) {
                ...
                $(nRow).hover(
                    function () {
                        $(this).addClass("xtr_hover");
                        appview.hover(this,aData);
                    },
                    function () {
                        $(this).removeClass("xtr_hover");
                        appview.refreshDetails(e.table);
                    }
                    );
                $(nRow).unbind('click');
                $(nRow).click( function() {
                    $(this).toggleClass('xtr_select');
                    $('img',this).toggle();
                    appview.refreshDetails(e.table,aData);
                });
                return nRow;
            },
            "fnDrawCallback": function ( oSettings ) {
                ...
                $.each(groups, function() {
                    $.each(this,function(){
                        $(".xtd_prio",this).addClass("prio" + color);
                    });
                    color++;
                });
        ...
        var refresh = function refresh() {
                    $.ajax({
                        ...
                        success: function(json) {
                            if(json['aaData'].length != G_Tables[group].fnGetData().length)
                                G_Tables[group].fnReloadAjax();
                        }
                    });
                }
...
var detailsBox = function detailsBox(appview,type) {
    ...
    this.detailstab = new detailstab(this.details,title);
    ...
    this.category = function (category,numSessions,title) {
        this.detailstab.changeName(title);
        this.detailswrap.showCategory(category,numSessions);
    }

    this.session = function(session,type) {
        this.detailstab.changeName(type);
        this.detailswrap.showSession(session,type);
    }

    this.classifier = function(aData) {
        this.detailstab.changeName("Resultados del entrenamiento");
        this.detailswrap.showClassifier(aData);
    }
...
var detailswrap = function detailswrap(father) {
    ...
    this.showSession = function(session,type) {
        if(type != 'Clasificador') {
            ...
            this.detailscontent.title.changeTitle(session);            
        }
    };

    this.showClassifier = function(aData) {
        this.detailscontent.title.changeTitle(aData[6]);
        this.detailscontent.listdetailswrap.classifier(aData);
    };
    ...
    this.showSessions = function(count) {
        ...
        this.detailscontent.listdetailswrap.classifierdetails.hide();
    };
...
var detailscontent = function detailscontent(father) {
    ...
    this.listdetailswrap = new listdetailswrap(this.element);
}
...
var listdetailswrap = function listdetailswrap(father) {
    this.element = $('<div id="listdetailswrap"></div>');
    this.classifierdetails = $('<div id="classifierdetails" style="display:none">\
                                    <div id="detailsinstances" style=""><span id="detailsinstances_highlight" title="">Número de instancias: <span class="field" id="detailsinstances_span"></span><a href=" " title=""><img class="field_img" src="/img/ico/ico_help_gry.gif" alt="Establecer fecha de vencimiento (d)"></a></span></div>\
                                    <div id="detailsattributes" style=""><span id="detailsattributes_highlight" title="">Número de atributos: <span class="field" id="detailsattributes_span"></span><a href=" " title=""><img class="field_img" src="/img/ico/ico_help_gry.gif" alt="Establecer fecha de vencimiento (d)"></a></span></div>\
                                    <div id="detailscorrect" style=""><span id="detailscorrect_highlight" title="">Clasificados correctamente: <span class="field" id="detailscorrect_span"></span><a href=" " title=""><img class="field_img" src="/img/ico/ico_help_gry.gif" alt="Establecer fecha de vencimiento (d)"></a></span></div>\
                                    <div id="detailspctcorrect" style=""><span id="detailspctcorrect_highlight" title="">Clasificados correctamente(%): <span class="field" id="detailspctcorrect_span"></span><a href=" " title=""><img class="field_img" src="/img/ico/ico_help_gry.gif" alt="Establecer fecha de vencimiento (d)"></a></span></div>\
                                    <div id="detailsincorrect" style=""><span id="detailsincorrect_highlight" title="">Clasificados incorrectamente: <span class="field" id="detailsincorrect_span"></span><a href=" " title=""><img class="field_img" src="/img/ico/ico_help_gry.gif" alt="Establecer fecha de vencimiento (d)"></a></span></div>\
                                    <div id="detailspctincorrect" style=""><span id="detailspctincorrect_highlight" title="">Clasificados incorrectamente(%): <span class="field" id="detailspctincorrect_span"></span><a href=" " title=""><img class="field_img" src="/img/ico/ico_help_gry.gif" alt="Establecer fecha de vencimiento (d)"></a></span></div>\
                                    <div id="detailsmae" style=""><span id="detailsmae_highlight" title="">Error media absoluto: <span class="field" id="detailsmae_span"></span><a href=" " title=""><img class="field_img" src="/img/ico/ico_help_gry.gif" alt="Establecer fecha de vencimiento (d)"></a></span></div>\
                                    <div id="detailsrmse" style=""><span id="detailsrmse_highlight" title="">Raiz del error cuadrático: <span class="field" id="detailsrmse_span"></span><a href=" " title=""><img class="field_img" src="/img/ico/ico_help_gry.gif" alt="Establecer fecha de vencimiento (d)"></a></span></div>\
                                    <div id="detailsrae" style=""><span id="detailsrae_highlight" title="">Error absoluto relativo: <span class="field" id="detailsrae_span"></span><a href=" " title=""><img class="field_img" src="/img/ico/ico_help_gry.gif" alt="Establecer fecha de vencimiento (d)"></a></span></div>\
                            </div>');
    this.element.append(this.classifierdetails);
    father.append(this.element);
    
    function twodecimal(floatstr) {
        var values = floatstr.split(".");
        return values[0] + "," + values[1].substr(0, 2);
    }
    this.classifier = function(aData) {
        $("#detailsinstances_span",this.classifierdetails).text(aData[7]);
        $("#detailsattributes_span",this.classifierdetails).text(aData[8]);
        $("#detailscorrect_span",this.classifierdetails).text(aData[9]);
        $("#detailspctcorrect_span",this.classifierdetails).text(twodecimal(aData[10]) + "%");
        $("#detailsincorrect_span",this.classifierdetails).text(aData[11]);
        $("#detailspctincorrect_span",this.classifierdetails).text(twodecimal(aData[12]) + "%");
        $("#detailsmae_span",this.classifierdetails).text(twodecimal(aData[13]));
        $("#detailsrmse_span",this.classifierdetails).text(twodecimal(aData[14]));
        $("#detailsrae_span",this.classifierdetails).text(twodecimal(aData[15]));
        this.classifierdetails.show();
    }
}
