...
var statusBox = function statusBox() {
    ...
    this.showMessage = function(text) {
        ...
        this.element.fadeIn();
        setTimeout("$('#statusbox').fadeOut(2000)", 5000);
    }
}
...
var appViewSes = function appViewSes(statusbox) {
    ...
    this.initialize = function() {
        $('#break').remove();
        $("#appview").remove();
        this.element = $('<div id="appview" style="display:none"></div>');
...
//        var selectedElements = V_Table.fnGetSelected('xtr_select');
        for ( var i=0 ; i<selectedData.length ; i++ )
        {
                $.getJSON('opinions.php',{
                    mode:"insert",
                    text:selectedData[i][1]
                },function(textStatus) {
                    if(textStatus == "success") console.log(selectedData[i][1]);
                });
            this.workElements.push(["",selectedData[i][1],""]);
...
var listWrap = function listWrap(appview,type) {
    this.element = $('<div id="listwrap"></div>');
...
//            alert(appview.selectedCategory.name);
    ...
    this.addA2W = function() {
        var addButton = $('<input type="submit" class="xtoolbox_button" value="A\u00F1adir elemento/os a conjunto de trabajo">');
...
/*    this.form.submit(function() {
        ...
        $.getJSON('member.php',{
            ...
            console.log("ahsjdgsadfkjsdf" + json);
            if(json == "success")
...
var detailshelp = function detailshelp(appview) {
    ...
    $('#detailsbox').append(this.element);
...
var session = function session(id,category,title,date,appview) {
    ...
    this.create = function() {
        ...
        this.element.click(function() {
            ...
            {
                s.selected = false;
                $(".xtd_check img",this).hide();
                appview.unselectSession(s);
            }
            else
            {
                s.selected = true;
                $(".xtd_check img",this).show();
