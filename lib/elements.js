...
var appViewSes = function appViewSes(statusbox) {
    ...
    this.addSession = function(title) {
        ...
        $.getJSON('modify.php',{
            ...
            else statusbox.showMessage('error','No ha sido posible realizar la operaci\u\u00F3n, int\u00E9ntelo otra vez por favor');
        });
    };
    ...
    this.deleteElements = function() {
        ...
        while(this.selectedSessions.length > 0) {
            ...
            $.getJSON('modify.php',{
                ...
                if(textStatus != "success") statusbox.showMessage('error','No ha sido posible realizar la operaci\u00F3n, int\u00E9ntelo otra vez por favor');
            });
...
var appViewTrain = function appViewTrain(statusbox) {
            ...
            for ( var i=0 ; i<selectedData.length ; i++ )
            {
                $.getJSON('opinions.php',{
                    ...
                    if(textStatus == (selectedData.length - 1)) {
                        ...
                        $(".dataTables_processing").hide();
                        appview.deleteElements(V_Table);
                    }
                });
            }
    ...
    this.changePolarity = function(polarity) {
        //        var selectedData = [];
        if($('.xtr_select >.xtd_polarity', G_polarityTable).length > 0) {
    ...
    this.createClassiffier = function() {
        var Data = [];
        $('.xtr',G_polarityTable).each(function() {
...
//            this.tools.addCC();
        ...
        if(type == 'polarity') {
            //            this.tools.addOpen();
            //            this.tools.addDelete();
            this.tools.addDelete(G_polarityTable);
            this.tools.addCP();
    ...
    this.polarityTable = function(options) {
        ...
        G_polarityTable = e.table.dataTable({
            ...
            },
            {
                "fnRender": function ( oObj ) {
                    ////console.log(oObj);
                    var select = '<form class="jqtransform"><span style="white-space: nowrap;"> <select class="polarity">';
