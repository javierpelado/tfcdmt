...
//        this.listbox.addTools('1');
    ...
    this.hover = function(s,aData) {
        //console.log(aData);
        //this.detailsbox.session($(".xtd_text",s).text(),'Clasificador');
        this.detailsbox.classifier(aData);
    }

    this.refreshDetails = function(table,aData) {
        var selectedElements = table.fnGetSelected('xtr_select');
        //console.log(table.fnGetData(selectedElements[0]));
            ...
            },
            {
                ...
                "fnRender" : function(oObj) {
                    if(oObj['aData'][16] < 100) {
                        ...
                        console.log(oObj);
//                        var check = setInterval("setPorcent",1000,);
                        redraw = true;
                        return cell;
                    }
                    else return getDateFormated(0,oObj['aData'][3]);
                }
            },
            {
                "aTargets":[6],
...
//                        //console.log(groups.size);
                ...
                $(".group",e.table).click();                
                if(redraw) {
                  setTimeout("G_Tables['" + group + "'].fnReloadAjax();",3000);            
                  redraw = false;
                }
                else $(".dataTables_processing",e.table).hide();
            }            
        });        
    }
}
...
var listdetailswrap = function listdetailswrap(father) {
    ...
    this.classifier = function(aData) {
        ...
        $("#detailspctcorrect_span",this.classifierdetails).text(aData[10] + "%");
        $("#detailsincorrect_span",this.classifierdetails).text(aData[11]);
        $("#detailspctincorrect_span",this.classifierdetails).text(aData[12] + "%");
